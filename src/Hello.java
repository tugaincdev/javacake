...
// Define a posição local do centro do olho esquerdo em relacao ao objeto
...
// usando a posição do objeto (pos) e o fator de escala
...
// Calcula a distância euclidiana entre o centro do olho e o mouse
            float distEsquerdo = dist(mouseX, mouseY, olhoEsquerdo_x_global, olhoEsquerdo_y_global);

// Define o limite máximo de movimento da pupila dentro do olho
            float distMax = 12f;
...
// Só move a pupila se houver distância entre olho e mouse
            if (distEsquerdo > 0) {
                // Limita o movimento da pupila ao valor máximo permitido
                float offsetGlobal = min(distEsquerdo, distMax);

                // Calcula o vetor unitário (direção normalizada) do olho para o mouse
                float unit_dx = dxEsquerdo / distEsquerdo;
                float unit_dy = dyEsquerdo / distEsquerdo;

                // Aplica o offset limitado na direção do mouse
                ...
                // Atualiza a posição final da pupila somando o offset ao centro do olho
